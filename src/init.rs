...
pub fn setinstance(instance: &str, config_file: &String) -> Result<()> {
    ...
    println!(
        "Remember to run 'source ~/{}' to apply changes",
        config_basename
    );
...
fn handle_defs_symlinks(instance: &str, config_file: &String) -> Result<()> {
    ...
    let p = format!(
        "export NIFTY_INSTANCE_{}_SERVER_DIR=",
    ...
    let p = format!(
        "export NIFTY_INSTANCE_{}_CLIENT_DIR=",
        instance.to_uppercase()
    );
    let client = &content
        .lines()
    ...
    symlink(
        Path::new(&server_path).join("defs.ts"),
        Path::new(&client_path).join("defs_server_symlink.ts"),
    )
    .unwrap_or_default();
    symlink(
        Path::new(&server_path).join("defs.ts"),
        Path::new(&instance_server_path).join("defs_server_symlink.ts"),
    )
    .unwrap_or_default();
    symlink(
        Path::new(&server_path).join("defs.ts"),
        Path::new(&instance_client_path).join("defs_server_symlink.ts"),
    )
    .unwrap_or_default();
    symlink(
        Path::new(&instance_server_path).join("defs.ts"),
        Path::new(&instance_client_path).join("defs_instance_server_symlink.ts"),
    )
    .unwrap_or_default();
    symlink(
        Path::new(&client_path).join("defs.ts"),
        Path::new(&instance_client_path).join("defs_client_symlink.ts"),
    )
    .unwrap_or_default();
...
pub fn find_config_file() -> Result<String> {
    ...
    let possible_configs = match (os_type, shell_type) {
        ...
        ("linux", "fish") => vec![format!("{}/{}", home_dir, ".config/fish/config.fish")],
        _ => vec![
            format!("{}/{}", home_dir, ".profile"),
            format!("{}/{}", home_dir, ".bashrc"),
        ],
