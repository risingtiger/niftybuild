...
pub fn runit(stats: &mut ProcessedStatsT) -> Result<()> {
    ...
    for entry in WalkDir::new(client_output_dev_path)
        .into_iter()
        .filter_entry(|e| is_skip_entry(e))
        .filter_map(|e| e.ok())
    {
        let path = entry.path();
...
fn process_js(file_in_path: &PathBuf, stats: &mut ProcessedStatsT) -> Result<()> {
    ...
    if file_in_path
        .components()
        .any(|component| component.as_os_str() == "lazy")
        && (file_in_path.components().any(|component| {
            component.as_os_str() == "views" || component.as_os_str() == "components"
        }))
    {
...
fn process_js_html_css_combined(js_file_str: &String, file_in_path: &Path) -> Result<String> {
    let css_file_str = fs::read_to_string(file_in_path.with_extension("css"))
        .unwrap_or_else(|_| String::from(" "));
    let html_file_str = fs::read_to_string(file_in_path.with_extension("html"))
        .unwrap_or_else(|_| String::from(" "));
    ...
    updated_js_file_str.push_str(
        &js_file_str
            .replace("{--css--}", &css_replacement_str)
            .replace("{--html--}", &html_replacement_str),
    );
